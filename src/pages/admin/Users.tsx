...
const Users = () => {
  ...
  const createUser = async () => {
    ...
    const r = await call({
      ...
      access_treinamento: cTrein, access_product: productFilter, access_mentoria: false,
    });
    setCreating(false);
    if (r) {
      toast.success("Usuário criado");
      setShowCreate(false);
      setCEmail(""); setCPassword(""); setCFullName(""); setCPhone(""); setCCpf(""); setCTrein(true);
      load();
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      const isPriv = u.roles.some((r) => ["admin", "master", "super_admin"].includes(r));
      if (productFilter && !isPriv && !hasAccessTo(u, productFilter)) return false;
      return true;
    });
  }, [users, search, filterTreinamento, filterPaidProduct, productFilter]);
