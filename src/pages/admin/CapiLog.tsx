...
const CapiLog = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      setBusy(true);
      let q = supabase.from("meta_capi_log").select("*").order("created_at", { ascending: false }).limit(100);
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-white/5 hover:bg-white/5">
                    ...
                    <td className="px-3 py-2 font-mono text-[10px] text-gray-400">{r.event_id?.slice(0, 8) || "-"}</td>
                    <td className="px-3 py-2 font-mono text-[10px] text-gray-400">{r.order_id?.slice(0, 8) || "-"}</td>
