...
const MembrosLogin = () => {
  ...
  // --- Alterar e-mail (modal de 2 passos) ---
  const [showCE, setShowCE] = useState(false);
  const [ceStep, setCeStep] = useState<1 | 2>(1);
  const [ceOld, setCeOld] = useState("");
  const [cePass, setCePass] = useState("");
  const [ceNew, setCeNew] = useState("");
  ...
  // Passo 1: valida e-mail atual + senha (modo verify-only da function).
  const ceContinue = async (e: React.FormEvent) => {
    e.preventDefault();
    setCeError("");
    if (!ceOld.trim() || !cePass) {
      setCeError("Preencha o e-mail atual e a senha.");
      return;
    }
    setCeLoading(true);
    const { data, error } = await supabase.functions.invoke("change-member-email", {
      body: { old_email: ceOld.trim().toLowerCase(), password: cePass },
    });
    setCeLoading(false);
    const d = data as { ok?: boolean; error?: string } | null;
    if (error || !d?.ok) {
      setCeError(d?.error === "invalid_credentials" ? "E-mail ou senha incorretos." : "Não foi possível validar agora. Tente novamente.");
      return;
    }
    setCeError("");
    setCeStep(2);
  ...
  // Passo 2: troca para o novo e-mail (mantém a senha).
  const ceSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setCeError("");
    const newEmail = ceNew.trim().toLowerCase();
    if (!newEmail) {
      setCeError("Digite o novo e-mail.");
      return;
    }
    if (newEmail === ceOld.trim().toLowerCase()) {
      setCeError("O novo e-mail é igual ao atual.");
      return;
    }
    setCeLoading(true);
    const { data, error } = await supabase.functions.invoke("change-member-email", {
      body: { old_email: ceOld.trim().toLowerCase(), password: cePass, new_email: newEmail },
    });
    setCeLoading(false);
    const d = data as { ok?: boolean; error?: string } | null;
    if (error || !d?.ok) {
      const map: Record<string, string> = {
        email_in_use: "Este e-mail já está cadastrado em outra conta.",
        same_email: "O novo e-mail é igual ao atual.",
        invalid_credentials: "Sua senha mudou? Refaça do início.",
      };
      setCeError((d?.error && map[d.error]) || "Não foi possível alterar agora. Tente novamente.");
      return;
    }
    // Sucesso: volta pro login com o novo e-mail preenchido, mesma senha.
    toast.success("E-mail alterado com sucesso! Entre com o novo e-mail e a mesma senha.");
    setEmail(newEmail);
    setPassword("");
    setShowCE(false);
  };
  ...
  return (
    <div className="relative min-h-screen bg-[#080808] flex flex-col">
      ...
      <div className="flex-1 flex items-start justify-center px-4 -mt-8 md:-mt-12 pb-12 relative z-10">
        <div className="bg-[#111] border border-white/10 rounded-xl p-6 max-w-md w-full shadow-2xl">
          ...
          <button
            ...
            className="w-full mt-2 text-xs text-gray-400 hover:text-[#00ff88]"
          >
            Alterar meu e-mail
          </button>
      ...
      {/* Alterar e-mail — modal de 2 passos */}
      {showCE && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => !ceLoading && setShowCE(false)}>
          <div className="bg-[#111] border border-white/10 rounded-xl p-6 w-full max-w-md relative shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowCE(false)} disabled={ceLoading} aria-label="Fechar" className="absolute top-3 right-3 text-gray-400 hover:text-white disabled:opacity-50">
              <X className="h-5 w-5" />
            </button>

            <h3 className="text-2xl font-bold text-white mb-1" style={{ fontFamily: "'Bebas Neue', cursive" }}>
              Alterar e-mail
            </h3>
            <p className="text-sm text-gray-400 mb-5">
              {ceStep === 1
                ? "Confirme seu e-mail atual e a senha para continuar."
                : "Digite o novo e-mail. Sua senha continua a mesma."}
            </p>

            {ceStep === 1 ? (
              <form onSubmit={ceContinue} className="space-y-4">
                <div>
                  <label className="text-xs text-gray-300 uppercase tracking-wider">E-mail atual</label>
                  <input value={ceOld} onChange={(e) => setCeOld(e.target.value)} type="email" className={inputCls} placeholder="seu@email.com" autoFocus />
                </div>
                <div>
                  <label className="text-xs text-gray-300 uppercase tracking-wider">Senha</label>
                  <input value={cePass} onChange={(e) => setCePass(e.target.value)} type="password" className={inputCls} placeholder="Sua senha" />
                </div>
                {ceError && <p className="text-xs text-[#ff2d78]">{ceError}</p>}
                <button type="submit" disabled={ceLoading} className="w-full h-12 rounded-md font-bold uppercase tracking-wide disabled:opacity-60" style={{ backgroundColor: "#00ff88", color: "#000", fontFamily: "'Bebas Neue', cursive" }}>
                  {ceLoading ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : "Continuar"}
                </button>
              </form>
            ) : (
              <form onSubmit={ceSave} className="space-y-4">
                <div>
                  <label className="text-xs text-gray-300 uppercase tracking-wider">Novo e-mail</label>
                  <input value={ceNew} onChange={(e) => setCeNew(e.target.value)} type="email" className={inputCls} placeholder="novo@email.com" autoFocus />
                </div>
                {ceError && <p className="text-xs text-[#ff2d78]">{ceError}</p>}
                <button type="submit" disabled={ceLoading} className="w-full h-12 rounded-md font-bold uppercase tracking-wide disabled:opacity-60" style={{ backgroundColor: "#00ff88", color: "#000", fontFamily: "'Bebas Neue', cursive" }}>
                  {ceLoading ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : "Salvar novo e-mail"}
                </button>
                <button type="button" onClick={() => { setCeStep(1); setCeError(""); }} disabled={ceLoading} className="w-full text-xs text-gray-400 hover:text-[#00ff88] disabled:opacity-60">
                  Voltar
                </button>
              </form>
            )}
