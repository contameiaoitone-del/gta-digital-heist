...
export function useTracking() {
  ...
  const saveLead = useCallback(
    async (data: { name?: string; email?: string; phone?: string; cpf?: string }) => {
      ...
      // Fire a Lead event (Pixel + CAPI + TikTok) with a shared event_id so the
      // funnel's "cadastros" are tracked by Meta/TikTok and show up in the CAPI
      // Log lead analytics (top UTM_CONTENT / UTM_CAMPAIGN per lead). Previously
      // saveLead only persisted the session, so Lead was invisible to Meta.
      const eventId = uuid();
      const eventIdTt = uuid();
      const pageUrl = window.location.href;
      const fbc = readFbc();
      ...
      fbq("track", "Lead", { content_name: "Treinamento" }, { eventID: eventId });
      ttq("Lead", { content_type: "product", content_id: "treinamento" }, { event_id: eventIdTt });
